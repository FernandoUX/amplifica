  ...
  return (
    <>
      ...
      <div className={`sm:hidden ${className}`}>
        <div className="relative">
          {/* Background line */}
          <div className="absolute top-[12px] left-0 right-0 flex items-center px-[14px]">
            <div className="w-full h-px bg-neutral-200" />
          </div>
          {progressPct > 0 && (
            <div className="absolute top-[12px] left-0 right-0 flex items-center px-[14px]">
              <div
                className="h-px bg-gradient-to-r from-neutral-400 to-primary-400"
                style={{ width: `${progressPct}%` }}
              />
            </div>
          )}
            ...
            {steps.map((step, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${styles.dot}`}
                  >
                    {step.status === "done" ? (
