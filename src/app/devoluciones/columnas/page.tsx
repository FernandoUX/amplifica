...
// ─── Devoluciones Column config (self-contained) ────────────────────────────
...
interface DevMovableCol {
  key: DevColumnKey;
  label: string;
}

const DEV_MOVABLE_COLS: DevMovableCol[] = [
  { key: "fechaCreacion",    label: "Fecha creacion" },
  { key: "fechaRecepcion",   label: "Fecha recepcion" },
  { key: "orderId",          label: "ID pedido" },
  { key: "seller",           label: "Seller" },
  { key: "sucursalOrigen",   label: "Sucursal origen" },
  { key: "ubicacionActual",  label: "Ubicacion actual" },
  { key: "origen",           label: "Origen" },
  { key: "courier",          label: "Courier" },
  { key: "estado",           label: "Estado" },
];

const DEV_DEFAULT_ORDER: DevColumnKey[] = DEV_MOVABLE_COLS.map(c => c.key);

// Default visible: hide origen and courier by default per UX finding
const DEV_DEFAULT_VISIBLE: DevColumnKey[] = [
  "fechaCreacion",
  "orderId",
  "seller",
  "sucursalOrigen",
...
const DEV_STORAGE_KEY  = "amplifica_devoluciones_cols_v1";
const DEV_CHANGE_EVENT = "amplifica_devoluciones_cols_change";

function readDevColStorage(): { order: DevColumnKey[]; visible: DevColumnKey[] } {
  if (typeof window === "undefined") {
    return { order: DEV_DEFAULT_ORDER, visible: DEV_DEFAULT_VISIBLE };
  }
  try {
    const raw = localStorage.getItem(DEV_STORAGE_KEY);
    if (!raw) return { order: DEV_DEFAULT_ORDER, visible: DEV_DEFAULT_VISIBLE };
    const parsed = JSON.parse(raw);
    const validKeys = new Set<DevColumnKey>(DEV_MOVABLE_COLS.map(c => c.key));
    const order = (parsed.order as DevColumnKey[]).filter(k => validKeys.has(k));
    const visible = (parsed.visible as DevColumnKey[]).filter(k => validKeys.has(k));
    // Add any missing keys at the end
    for (const k of DEV_DEFAULT_ORDER) {
      if (!order.includes(k)) order.push(k);
    }
    return {
      order,
      visible: visible.length > 0 ? visible : DEV_DEFAULT_VISIBLE,
    };
  } catch {
    return { order: DEV_DEFAULT_ORDER, visible: DEV_DEFAULT_VISIBLE };
  }
...
// ─── Page ───────────────────────────────────────────────────────────────────
export default function DevolucionesColumnEditorPage() {
  const router = useRouter();

  // Init from localStorage
  const initial = readDevColStorage();
  const [order,   setOrder]   = useState<DevColumnKey[]>(initial.order);
  const [visible, setVisible] = useState<Set<DevColumnKey>>(new Set(initial.visible));
  const [density, setDensity] = useState<"compact" | "comfortable">(() => {
    if (typeof window === "undefined") return "compact";
    return (localStorage.getItem("amplifica_devoluciones_density") as "compact" | "comfortable") || "compact";
  });

  // DnD state
  const [dragging,    setDragging]    = useState<DevColumnKey | null>(null);
  const [dragOverKey, setDragOverKey] = useState<DevColumnKey | null>(null);
  const [dropBefore,  setDropBefore]  = useState(true);

  // ── Visibility toggle ──
  const toggleVisible = (key: DevColumnKey) => {
    setVisible(prev => {
  ...
  const allKeys = DEV_MOVABLE_COLS.map(c => c.key);
  const selectAll = () => setVisible(new Set(allKeys));
  const clearAll  = () => {
    const first = order[0] ?? DEV_DEFAULT_ORDER[0];
    setVisible(new Set([first]));
  };

  // ── Drag handlers ──
  const handleDragStart = (e: React.DragEvent, key: DevColumnKey) => {
    e.dataTransfer.effectAllowed = "move";
    setDragging(key);
  };

  const handleDragOver = (e: React.DragEvent, key: DevColumnKey) => {
    e.preventDefault();
  ...
  const handleDrop = (e: React.DragEvent, targetKey: DevColumnKey) => {
    e.preventDefault();
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("amplifica_devoluciones_density", density);
    window.dispatchEvent(new CustomEvent(DEV_CHANGE_EVENT));
    window.dispatchEvent(new CustomEvent("amplifica_devoluciones_density_change"));
    router.push("/devoluciones");
  };

  const handleReset = () => {
    setOrder(DEV_DEFAULT_ORDER);
    setVisible(new Set(DEV_DEFAULT_VISIBLE));
  };
    ...
    <>
      ...
      <div>
        <nav className="max-w-5xl mx-auto px-4 lg:px-6 py-3 flex items-center gap-1.5 text-sm text-neutral-500">
          <Link href="/devoluciones" className="hover:text-primary-500 transition-colors duration-300">Devoluciones</Link>
    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto pb-36 sm:pb-4 lg:pb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-5 gap-3 sm:gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-neutral-900">Editor de columnas — Devoluciones</h1>
            <p className="text-sm text-neutral-500 mt-0.5">
              Personaliza las columnas de la tabla de Devoluciones
            </p>
          </div>
        </div>
        {/* Desktop buttons */}
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-5 mb-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-neutral-500 mt-0.5">
              {visibleCount} de {DEV_MOVABLE_COLS.length} columnas activas
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2">
          {/* Fixed: ID devolucion — always visible */}
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-5">
        <div className="mb-4">
          ...
          <p className="text-xs text-neutral-500 mt-0.5">
            Arrastra para reordenar. Las columnas ID devolucion y Acciones son fijas.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:flex-wrap">
          ...
          <div className="flex items-center gap-2 px-3 py-2.5 bg-neutral-50 border border-dashed border-neutral-200 rounded-xl text-sm text-neutral-600 w-full sm:w-auto">
            <LockIcon />
            <span>ID devolucion</span>
          </div>

          {/* Draggable columns */}
          {order.map(key => {
            const col = DEV_MOVABLE_COLS.find(c => c.key === key);
            if (!col) return null;
